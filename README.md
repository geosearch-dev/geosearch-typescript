...
> **This package is not published to npm.** Installing it by its package name
> will fail with E404 — install from this repository instead.

```bash
npm install github:geosearch-dev/geosearch-typescript
```

To pin a release rather than tracking `main`, append a tag:

```bash
npm install github:geosearch-dev/geosearch-typescript#v1.0.0
```
