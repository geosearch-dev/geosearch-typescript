...
> PostalCodeListResponse listPostalCodes(lang, country, code, within, bbox, cursor, limit, fields, sort)
...
async function example() {
  ...
  const api = new PostalCodesApi(config);
  ...
  const body = {
    ...
    country: US,
    // string | Filter by postal code (optional)
    code: 94105,
    ...
    // string | Sort field. Allowed: postal_code, country_code, place_name, id. (optional)
    sort: postal_code,
  } satisfies ListPostalCodesRequest;

  try {
    const data = await api.listPostalCodes(body);
    console.log(data);
  } catch (error) {
    console.error(error);
  }
...
| **code** | `string` | Filter by postal code | [Optional] [Defaults to `undefined`] |
...
| **sort** | `string` | Sort field. Allowed: postal_code, country_code, place_name, id. | [Optional] [Defaults to `undefined`] |
...
> PostalCodeListResponse nearestPostalCode(lat, lon, lang, limit)
...
async function example() {
  ...
  const body = {
    ...
    // string | ISO 639-1 language code for localized names (e.g., de, fr, ja) (optional)
    lang: de,
    // number | Number of results (1-10, default 1) (optional)
    limit: 3,
  } satisfies NearestPostalCodeRequest;

  try {
    const data = await api.nearestPostalCode(body);
    console.log(data);
  } catch (error) {
    console.error(error);
  }
...
| **lang** | `string` | ISO 639-1 language code for localized names (e.g., de, fr, ja) | [Optional] [Defaults to `undefined`] |
| **limit** | `number` | Number of results (1-10, default 1) | [Optional] [Defaults to `1`] |
